...
class DrivingEfficiencyScorer {
    ...
    private fun calculateCruisingScore(data: DriveSessionData): Int {
        if (data.totalSamples == 0) return 50
        var score = 80
        val optimalSpeedRatio = if (data.avgSpeed in OPTIMAL_SPEED_MIN..OPTIMAL_SPEED_MAX) 1.0
        else {
            val deviation = if (data.avgSpeed < OPTIMAL_SPEED_MIN) (OPTIMAL_SPEED_MIN - data.avgSpeed) / OPTIMAL_SPEED_MIN
            else (data.avgSpeed - OPTIMAL_SPEED_MAX) / OPTIMAL_SPEED_MAX
    ...
    private fun calculateBrakingScore(data: DriveSessionData): Int {
        ...
        if (data.avgSpeed > 80 && data.harshBrakes > 0) score -= 10
        return score.coerceIn(0, 100)
    }
    ...
    private fun calculateThrottleScore(data: DriveSessionData): Int {
        ...
        if (data.maxThrottle > 90) score -= 10
        return score.coerceIn(0, 100)
    }

    fun generateTips(score: EfficiencyScore, data: DriveSessionData): List<String> {
        val tips = mutableListOf<String>()
        if (score.accelerationScore < 70) tips.add("Gaspedal gleichmäßiger betätigen")
        if (score.cruisingScore < 70) tips.add("Geschwindigkeit zwischen 50-110 km/h halten")
        if (score.brakingScore < 70) tips.add("Vorausschauend fahren - Bremsungen vermeiden")
        if (score.rpmEfficiencyScore < 70) tips.add("Drehzahl im Bereich 1500-3000 halten")
        if (data.idleTimePercent > 20) tips.add("Motor im Stand nicht im Leerlauf lassen")
        if (tips.isEmpty()) tips.add("Fahrstil ist effizient!")
        return tips.take(4)
    }
